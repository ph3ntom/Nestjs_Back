...
export class AnswerService {
  ...
  async create(questionId: number, createAnswerDto: CreateAnswerDto, userId: number): Promise<AnswerResponseDto> {
  ...
  async update(id: number, updateAnswerDto: UpdateAnswerDto, userId: number): Promise<AnswerResponseDto> {
    const answer = await this.answerRepository.findOne({
      where: { id },
      relations: ['user'],
    });
  ...
  async remove(id: number, userId: number): Promise<void> {
    ...
    await this.questionRepository.decrement({ id: answer.questionId }, 'answers', 1);
  }
  ...
  async markAsAccepted(id: number, questionOwnerId: number): Promise<AnswerResponseDto> {
    const answer = await this.answerRepository.findOne({
      where: { id },
    ...
    if (answer.question.userId !== questionOwnerId) {
      throw new ForbiddenException('Only the question owner can accept answers');
    }
