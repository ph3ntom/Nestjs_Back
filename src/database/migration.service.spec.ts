...
describe('MigrationService', () => {
  ...
  describe('getMigrationStatus', () => {
    ...
    it('should handle database query errors', async () => {
      ...
      await expect(service.getMigrationStatus()).rejects.toThrow('Database error');
    });
  });
  ...
  describe('validateUserData', () => {
    ...
    it('should detect missing test users', async () => {
      ...
      expect(result.errors).toContain('Missing test users: testuser2, testuser3, developer, tester');
    });
    ...
    it('should handle validation errors gracefully', async () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors[0]).toContain('Validation failed: Database connection failed');
    });
  });
  ...
  describe('testMigrations', () => {
    it('should test all pending migrations successfully', async () => {
      ...
      jest.spyOn(service, 'getMigrationStatus').mockResolvedValue(mockMigrationStatus);
      ...
      const executedMigration = result.results.find(r => r.migration === 'AddTestUsers1722000001000');
      expect(executedMigration?.status).toBe('success');
      
      const skippedMigration = result.results.find(r => r.migration === 'InitialMigration1722000000000');
    ...
    it('should handle migration execution failures', async () => {
      const mockMigrationStatus = {
        executed: [],
        pending: [
          { name: 'FailingMigration', timestamp: 1722000002000 }
        ]
      };

      jest.spyOn(service, 'getMigrationStatus').mockResolvedValue(mockMigrationStatus);
      jest.spyOn(service, 'runMigrationsTo').mockRejectedValue(new Error('Migration failed'));
