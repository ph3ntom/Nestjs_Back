...
export class MigrationController {
  ...
  async fullDatabaseTest() {
    try {
      ...
      this.logger.log(
        `✓ Migration test completed: ${migrationTest.summary.executed} executed, ${migrationTest.summary.failed} failed`,
      );

      // 3. 사용자 데이터 검증
      const userValidation = await this.migrationService.validateUserData();
      this.logger.log(
        `✓ User validation completed: ${userValidation.totalUsers} users found`,
      );
      ...
      const overallSuccess =
        migrationTest.success && userValidation.isValid && dbHealth.isConnected;
  ...
  async seedQuestions() {
    try {
      ...
      const testQuestions = [
        ...
        {
          title:
            'SignalR in dotnet 9 and Angular v19 errors - Websockets issues',
          description:
      ...
      const questions =
        this.testDataService.questionRepository.create(testQuestions);
      await this.testDataService.questionRepository.save(questions);

      // 답변은 seedTestQuestions에서 자동으로 생성됩니다
      this.logger.log('Test answers are created automatically with questions');
  ...
  /**
   * 답변 테스트 데이터 생성 (deprecated - answers are now created with questions)
   */
  @Post('seed-answers')
  async seedAnswers() {
    try {
      this.logger.log('Answers are now created automatically with questions');

      return {
        success: true,
        message:
          'Answers are created automatically with questions. Use /seed endpoint instead.',
      };
    } catch (error) {
